    ...
    getContent(p) {
      ...
      return {
        term: key === 'buyer' ? `<strong>Annual economic curtailment hour cap: 200 hours/year.</strong> Seller may not invoke economic curtailment protections (non-settlement, deemed generation) for more than 200 hours in any contract year. Hours exceeding cap are settled normally regardless of floating price. Cap resets annually. Trigger must be automatic — buyer may not direct curtailment.` :
              key === 'market' ? `<strong>No annual curtailment hour cap (uncapped).</strong> CEBA standard for corporate VPPAs: curtailment hour caps are common in utility PPAs but have historically been rare in corporate deals — though increasingly appearing as negative pricing frequency rises. Negative price protection mechanism (Options 1/1a/2) applies without hour limit.` :
