...
// Market-standard VPPA term analysis, v3.0
...
const TERM_META = {
  ...
  // Shape & Curtailment
  curtailment:    { name: 'Economic Curtailment Hour Cap',      category: 'Negative Price Risk',       icon: '⚡', defaultPos: 0, group: 'shape',    flexibility: 'flexible' },
...
const CONTENT = {
  ...
  strike: {
    getContent(p) {
      ...
      return {
        ...
        bench: `Market range: Min ${DEAL.benchmarkMin} / Avg ${DEAL.benchmarkAvg} / Max ${DEAL.benchmarkMax}. Typical term: 12–20 years; 15 years is pricing benchmark.`,
  ...
  floating: {
    getContent(p) {
      ...
      return {
        ...
        bench: `Market standard: ISO real-time or day-ahead LMP at Hub, with market-settlement interval identical to ISO (hourly for day-ahead; 15-min for real-time). No seller election rights. Day-ahead vs real-time should be explicitly analyzed — each has different volatility profile.`,
    ...
    getContent(p) {
      ...
      return {
        term: key === 'buyer' ? `<strong>15-minute settlement interval</strong> matching ${iso} real-time dispatch. Maximum granularity — tracks actual generation and pricing with no aggregation smoothing.` :
              key === 'market' ? `<strong>Hourly settlement interval</strong> matching ${iso} market-settlement interval. market standard — identical to ISO real-time hourly pricing.` :
        ...
        bench: `Market standard: market-settlement interval identical to ISO. For day-ahead markets, hourly is standard. For real-time, 15-minute intervals may be available. Monthly averaging is seller-favorable and non-standard.`,
        impact: key === 'buyer' ? '15-MINUTE RT' : key === 'market' ? 'HOURLY — STANDARD' : key === 'seller' ? 'DAILY / MONTHLY' : 'UNDEFINED',
        impactsub: key === 'buyer' ? '15-min granularity<br>matches ISO dispatch' : key === 'market' ? 'Hourly, ISO-matched<br>market standard' : key === 'seller' ? 'Aggregated intervals<br>hide unfavorable hours' : 'No interval defined<br>dispute risk',
  ...
  negprice: {
    getContent(p) {
      ...
      return {
        ...
        bench: `Market standard: Floor Price = $0 for solar; $0 minus grossed-up PTC value for wind in years 1–10, then $0. Option 2 (zero floor) is most buyer-friendly — project keeps generating, buyer keeps RECs. Option 1 (non-settlement) is market standard. Option 1a is wind-specific PTC preservation. No protection is a hard red flag.`,
    ...
    getContent(p) {
      ...
      return {
        term: key === 'buyer' ? `<strong>Seller invoices by 10th business day of following month.</strong> Buyer pays within 20 days of receipt. Net 20 effective cycle. Buyer specifies invoice format, supporting data, and calculation methodology in contract.` :
              key === 'market' ? `<strong>Seller invoices by 15th day of following month.</strong> Buyer pays within 30 days of receipt; seller pays buyer within 30 days after contract month end. market standard Net 30. Note: ISO/RTO restatements may delay settlement calculations.` :
        ...
        bench: `Market standard: Seller invoices by 15th of following month; buyer pays Net 30; seller pays buyer Net 30 after month end. Buyers should specify invoice format and supporting data during negotiation. Some sellers include interest on late payments — negotiate cap at prime + 1%.`,
        impact: key === 'buyer' ? 'NET 20' : key === 'market' ? 'NET 30 — STANDARD' : key === 'seller' ? 'NET 45+ / INTEREST' : 'UNDEFINED',
        impactsub: key === 'buyer' ? 'Fast cycle + defined format<br>low dispute risk' : key === 'market' ? '15th invoice, 30-day pay<br>market standard' : key === 'seller' ? 'Extended or punitive<br>interest on late pay' : 'No timeline defined<br>high dispute risk',
    ...
    getContent(p) {
      ...
      return {
        term: key === 'buyer' ? `<strong>Hub settlement at ${iso} Hub.</strong> Seller bears all basis risk between project node and hub — seller is in the best position to assess and hedge this via owned instruments. Basis blowout language included: if hub exceeds node by 2x fixed price, seller may not curtail without financial consequence.` :
              key === 'market' ? `<strong>Hub settlement at ${iso} Hub.</strong> Seller bears hub-to-node spread. Hub settlement increases PPA strike price vs node settlement but limits basis and congestion risk for buyer. Most buyers prefer paying the premium for hub certainty.` :
              ...
              `<strong>Seller election between hub and node per interval.</strong> Deal structure Option 1: seller can shift floating price to (busbar + fixed price) when settlement is negative AND busbar > $0 AND floating > (busbar + 2x fixed). Severe buyer exposure.`,
        bench: `Market standard: Hub settlement. Seller is best positioned to assess and bear basis risk via hedging instruments — and will price this into strike. Node settlement transfers a risk to buyer that buyer cannot independently hedge. Hub-to-node spread in ERCOT averages $2–5/MWh but can spike to $20+/MWh during congestion.`,
        impact: key === 'buyer' ? 'HUB + BLOWOUT CLAUSE' : key === 'market' ? 'HUB — STANDARD' : key === 'seller' ? 'NODE EXPOSURE' : 'SELLER ELECTION',
        impactsub: key === 'buyer' ? 'Hub + congestion protection<br>seller bears basis risk' : key === 'market' ? 'Hub LMP, market standard<br>seller bears spread' : key === 'seller' ? '$2–5/MWh avg basis drag<br>buyer cannot hedge' : 'Seller picks best point<br>per-interval cherry-pick',
        rec: key === 'red' ? 'Reject seller election. Fix at Hub. Market practice: use Basis LMP formula (Option 2) — if spread < $X, Basis LMP = Hub; if spread ≥ $X, Basis LMP = greater of 2x contract price or real-time nodal LMP.' : key === 'seller' ? 'Push for Hub settlement. Node settlement transfers unhedgeable basis risk to buyer. Most buyers pay the strike premium for hub certainty.' : 'Hub settlement acceptable. Consider adding basis blowout clause for extreme congestion events.',
    ...
    getContent(p) {
      ...
      return {
        term: key === 'buyer' ? `<strong>No market disruption clause.</strong> All settlement at published ISO prices regardless of market conditions. If disruption occurs, parties retroactively settle using historic average pricing with true-up.` :
              ...
              `<strong>Market disruption with seller-defined fallback pricing.</strong> Seller sets reference price during disruption periods. Market disruptions typically do not rise to Force Majeure level and should be separately negotiated — seller-controlled pricing is unacceptable.`,
        bench: `Market standard: Good-faith negotiation on replacement formula if floating price becomes unavailable or materially changes. Historic average pricing with true-up is common fallback. Seller-calculated reference prices are non-standard. Market disruption language may overlap with Force Majeure — ensure no double coverage.`,
    ...
    getContent(p) {
      ...
      return {
        term: key === 'buyer' ? `<strong>Seller acts as market participant / Qualified Scheduling Entity (QSE)</strong> and bears all costs for maintaining its ISO/RTO account, scheduling, and transmission-related charges. Seller may not outsource QSE function without buyer consent.` :
              key === 'market' ? `<strong>Seller acts as market participant / QSE at ${iso}</strong> and is responsible for all costs for maintaining its account with the ISO/RTO. Standard market-aligned allocation — seller is best positioned to manage scheduling.` :
        ...
        bench: `Market standard (inflexible term): Seller acts as market participant/QSE and bears all ISO/RTO account costs. This is an inflexible term — seller should always bear scheduling responsibility. Some sellers outsource to third-party QSEs; buyer should retain consent right.`,
        impact: key === 'buyer' ? 'SELLER QSE — LOCKED' : key === 'market' ? 'SELLER QSE — STANDARD' : key === 'seller' ? 'THIRD-PARTY QSE' : 'NO ALLOCATION',
        impactsub: key === 'buyer' ? 'Seller bears all ISO costs<br>no outsource without consent' : key === 'market' ? 'Seller as QSE<br>market inflexible standard' : key === 'seller' ? 'Third-party QSE possible<br>cost/liability gap' : 'ISO costs unallocated<br>buyer exposure risk',
        rec: key === 'red' ? 'Add explicit scheduling provision. Seller must act as QSE and bear all ISO/RTO account costs. Undefined scheduling allocation creates imbalance charge exposure. This is market-standard as an inflexible term.' : key === 'seller' ? 'Add consent right for QSE outsourcing. Third-party QSE without buyer consent creates accountability gap for scheduling errors.' : 'Scheduling allocation acceptable.',
  ...
  curtailment: {
    getContent(p) {
      ...
      const iso = DEAL.iso || 'ERCOT';
      const isERCOT = iso.toUpperCase().includes('ERCOT');
      const negHoursEstimate = isERCOT ? '300–800 hours/year' : '100–400 hours/year';
      return {
        term: key === 'buyer' ? `<strong>Annual economic curtailment hour cap: 200 hours/year.</strong> Seller may not invoke economic curtailment protections (non-settlement, deemed generation) for more than 200 hours in any contract year. Hours exceeding cap are settled normally regardless of floating price. Cap resets annually. Trigger must be automatic — buyer may not direct curtailment.` :
              key === 'market' ? `<strong>No annual curtailment hour cap (uncapped).</strong> market standard for corporate VPPAs: curtailment hour caps are common in utility PPAs but have historically been rare in corporate deals — though increasingly appearing as negative pricing frequency rises. Negative price protection mechanism (Options 1/1a/2) applies without hour limit.` :
              key === 'seller' ? `<strong>Annual curtailment hour cap set at 500+ hours/year</strong> — effectively uncapped given typical negative pricing frequency. Cap is too high to provide meaningful protection against seller systematically avoiding settlement during unfavorable hours. ${isERCOT ? `ERCOT sees ${negHoursEstimate} of negative pricing in active zones.` : ''}` :
              `<strong>No curtailment hour cap AND no automatic trigger requirement.</strong> Seller may elect curtailment at will during any negative price period, with buyer potentially having influence over curtailment decisions — creating regulatory exposure. ${isERCOT ? `In ERCOT, uncapped seller curtailment election across ${negHoursEstimate} of negative pricing can represent material lost REC delivery.` : 'Uncapped seller curtailment election creates both financial and regulatory risk for buyer.'}`,
        bench: `Market standard: \"The term sheet could provide a cap in terms of hours of economic curtailment, although while common in utility PPAs it has historically been rare in corporate VPPAs until recently.\" As negative pricing frequency increases in ERCOT and other markets, hour caps are gaining traction. Key Key requirement: any curtailment clause must use an automatic trigger — buyer must NOT have the right to direct curtailment (regulatory risk). Negative price protection structure (Options 1/1a/2) is a separate, complementary term.`,
        impact: key === 'buyer' ? '200-HR/YR CAP' : key === 'market' ? 'UNCAPPED — STANDARD' : key === 'seller' ? '500+ HR CAP — WEAK' : 'NO CAP + NO AUTO TRIGGER',
        impactsub: key === 'buyer' ? '200-hr annual cap<br>auto trigger only' : key === 'market' ? 'No cap, market standard<br>neg price protection applies' : key === 'seller' ? 'High cap = effectively uncapped<br>limited buyer protection' : 'Uncapped + buyer direction<br>financial + regulatory risk',
        rec: key === 'red' ? `Add automatic trigger requirement — buyer must not direct curtailment. Consider negotiating annual hour cap (200–300 hrs) as negative pricing frequency increases. ${isERCOT ? 'ERCOT negative pricing is frequent enough that an uncapped, buyer-directed curtailment right creates material regulatory exposure.' : ''}` : key === 'seller' ? 'Reduce cap to 300 hours maximum or push to full uncapped market standard structure. A 500+ hr cap provides no meaningful protection in high-negative-pricing markets. Ensure auto-trigger mechanism is explicit.' : 'Curtailment structure acceptable. Confirm automatic trigger is specified — buyer direction of curtailment creates regulatory risk regardless of cap level.',
        reclabel: key === 'red' ? 'Priority — Add Auto Trigger' : key === 'seller' ? 'Priority — Tighten Cap' : 'Outcome — Acceptable'
      };
    }
  },
    ...
    getContent(p) {
      ...
      return {
        term: key === 'buyer' ? `<strong>Non-economic curtailment narrowly defined.</strong> No swap or settlement occurs only when the project is curtailed or removed from service due to: (1) system emergency, (2) forced outage, or (3) ISO/RTO-directed curtailment. Force majeure events separately addressed. No RECs delivered during non-economic curtailment hours.` :
              key === 'market' ? `<strong>Standard non-economic curtailment provision.</strong> No settlement during system emergencies, forced outages, or ISO/RTO-directed curtailment. market standard language — distinct from economic curtailment (negative prices) and force majeure. No RECs for curtailed hours.` :
        ...
        bench: `Market standard: Non-economic curtailment = no settlement during system emergencies, forced outages, and ISO/RTO-directed curtailment ONLY. Explicitly separate from (1) economic curtailment (negative price events) and (2) force majeure. Seller maintenance should not qualify. No RECs delivered during non-economic curtailment hours.`,
        impact: key === 'buyer' ? 'NARROW DEFINITION' : key === 'market' ? 'MARKET STANDARD' : key === 'seller' ? 'BROAD — SELLER ELECTION' : 'UNDEFINED / CONFLATED',
        ...
        rec: key === 'red' ? 'Define non-economic curtailment explicitly and separately from force majeure. Market standard: only system emergencies, forced outages, and ISO-directed curtailment qualify. Seller-elected downtime should not excuse settlement.' : key === 'seller' ? 'Narrow definition. Remove seller-elected maintenance from non-economic curtailment — maintenance should be planned and cannot be used to avoid settlement obligations.' : 'Non-economic curtailment provision acceptable. Confirm it is separate from FM and economic curtailment definitions.',
  ...
  basiscurtail: {
    getContent(p) {
      ...
      const isERCOT = iso.toUpperCase().includes('ERCOT');
      return {
        term: key === 'buyer' ? `<strong>Deemed generation during basis curtailment hours.</strong> If project is ISO-dispatched offline due to negative nodal/busbar price while Hub LMP is positive, seller settles as if generating at Hub price. RECs delivered for deemed hours. Market deal structure — Basis LMP formula: if spread < $X → Basis LMP = Hub; if spread ≥ $X → Basis LMP = greater of (2x contract price) or real-time nodal LMP.` :
  ...
  ia: {
    getContent(p) {
      ...
      return {
        ...
        bench: `Standard: Executed IA as CP, or IA CP with buyer termination right if not satisfied within 90–180 days. Buyer credit support and project financing requirements make interconnection status the single highest COD risk factor. Projects without IAs should carry aggressive delay damages.`,
  ...
  cp: {
    getContent(p) {
      ...
      return {
        ...
        bench: `Market standard: Executed IA + major permits as CPs with outside dates (12–18 months). No CPs means buyer is locked into a potentially stranded project. Contracts should also address scenarios where developer sells the project — buyer should negotiate step-in rights and credit support continuity.`,
    ...
    getContent(p) {
      ...
      return {
        term: key === 'buyer' ? `<strong>$250/MW/day Delay Damages from TCOD.</strong> Shortfall Damages: $100K/MW for uninstalled capacity at Final Nameplate Capacity true-up (6 months post-COD). OCOD = TCOD + 180 days with automatic buyer termination right. REC damages may be included in lieu of or in addition to cash.` :
              key === 'market' ? `<strong>$150–200/MW/day Delay Damages from TCOD.</strong> Shortfall Damages at Final Nameplate Capacity true-up. OCOD = TCOD + 180 days with buyer termination right. VPPAs have double value proposition — buyer may request RECs in lieu of or in addition to cash delay damages to maintain sustainability goals.` :
        ...
        bench: `Market standard: $150–250/MW/day Delay Damages from TCOD. OCOD = TCOD + 6 months with buyer termination right. Shortfall Damages for uninstalled capacity at Final Nameplate Capacity true-up. Buyers may request REC damages in addition to cash — critical for sustainability reporting deadlines.`,
    ...
    getContent(p) {
      ...
      return {
        term: key === 'buyer' ? `<strong>${isWind ? '92%' : '97%'} mechanical availability guarantee,</strong> measured on rolling 2-consecutive-year basis. Meaningful LD structure: (Average Energy Value + Average REC Value) × Buyer's Share × (Guaranteed % − Actual %). Seasonal restriction on maintenance during high-value months.` :
              key === 'market' ? `<strong>${isWind ? '90%' : '94%'} first year, ${twoYearStd}% in subsequent years</strong> (or ${isWind ? '92%' : '95%'} on any 2 consecutive years). Market standard: LD = sum of (Average Energy Value + Average REC Value) × Buyer's Share × shortfall percentage. Seasonal maintenance restriction applies.` :
              key === 'seller' ? `<strong>Availability guarantee ${weakGuarantee}</strong> or LD payment is nominal. Developers may offer slightly depressed availability percentages to build in cushion — buyer should push back.` :
  ...
  availguaranteed: {
    getContent(p) {
      ...
      const isWind = tech.includes('wind');
      return {
        term: key === 'buyer' ? `<strong>No Guaranteed Annual Production volume.</strong> Market consensus strongly recommends buyers avoid guaranteed production volumes. Guaranteed production (fixed MWh/year) can trigger derivative accounting treatment under US GAAP — requiring contract capitalization and balance sheet recognition.` :
              key === 'market' ? `<strong>No Guaranteed Annual Production.</strong> Buyer's Share expressed as percentage of output, not fixed MWh. Market standard: percentage-of-capacity structure de-risks both parties — buyer avoids derivative accounting, seller avoids fixed output obligation versus variable generation.` :
        ...
        bench: `Most buyers AVOID Guaranteed Annual Production because it can trigger derivative accounting treatment under US GAAP. Buyer's Share should be expressed as a % of Planned Nameplate Capacity, not fixed MWh. Cap Buyer's Share at 90% to avoid lease accounting treatment. Consult industry accounting guidance and Big 4 advisor before accepting any production guarantee.`,
        ...
        impactsub: key === 'buyer' ? 'No production guarantee<br>avoids derivative accounting' : key === 'market' ? '% of capacity structure<br>market preferred, no acct risk' : key === 'seller' ? 'Fixed MWh guarantee<br>confirm accounting treatment' : 'Fixed MWh structure<br>possible derivative accounting',
  ...
  permit: {
    getContent(p) {
      const key = p <= 25 ? 'buyer' : p <= 50 ? 'market' : p <= 75 ? 'seller' : 'red';
      return {
        term: key === 'buyer' ? `<strong>New permitting requirements are not force majeure events.</strong> Seller bears full permitting risk with no renegotiation rights. Interconnection and permitting delays are developer execution risks — market standard explicitly excludes these from FM definitions.` :
              key === 'market' ? `<strong>New permitting requirements trigger 30-day good-faith renegotiation only.</strong> If parties cannot agree, either may terminate without damages. Market standard: changes in law should not allow either party to unilaterally renegotiate fixed price terms.` :
        ...
        bench: `Market standard: Permitting delays are NOT force majeure events and should not allow seller to renegotiate or exit. Standard: mutual termination right after 30-day good-faith renegotiation fails, no damages to either party. Unilateral seller exit on permitting change is non-standard and buyer-adverse.`,
        ...
        rec: key === 'red' ? 'Reject unilateral seller exit. Any permitting provision must result in mutual termination right only, no damages. Market standard explicitly excludes permitting delays from FM — seller should bear development execution risk.' : key === 'seller' ? 'Remove seller suspension right. Replace with mutual termination after 30-day renegotiation fails. Permitting risk belongs to seller.' : 'Permitting provision acceptable.',
  ...
  cod: {
    getContent(p) {
      ...
      return {
        ...
        bench: `Market standard: TCOD defined; OCOD = TCOD + 6 months; buyer termination right at OCOD. COD conditions: 90% of Planned Nameplate Capacity + IE certification. Final Nameplate Capacity true-up at OCOD date with LDs per MW shortfall. Buyer's Share capped at 90% for lease accounting avoidance. Sustainability teams are responsible for internal deadlines — OCOD protection is critical.`,
    ...
    getContent(p) {
      ...
      return {
        term: key === 'buyer' ? `<strong>Letter of Credit from Qualified Institution (A-/A3 rated bank, $2.5B+ net worth).</strong> Seller Development Security: $100K/MWac pre-COD. Operational Security: $75K/MWac post-COD. Provided within 30 days of VPPA execution. Replenishment within 5 business days of draw.` :
              key === 'market' ? `<strong>LC from Qualified Institution or Parent Guarantee from Required Credit Rating entity</strong> (BBB-/Baa3 or better). Seller Development Security: $100K/MWac pre-COD. $75K/MWac post-COD. Market standard: LC preferred during development phase when risk is highest. Parent guarantee acceptable if parent meets Required Credit Rating.` :
              ...
              `<strong>No seller performance assurance.</strong> Since seller is an SPV/ProjectCo LLC with no other assets, buyer has zero financial protection against seller default, project abandonment, or failure to achieve COD. This is the most critical credit protection for buyer.`,
        bench: `Market standard: Seller PA = $100K/MWac pre-COD (Development Security); $75K/MWac post-COD. Qualified Institution = A-/A3 rated bank, $2.5B+ net worth. Seller entities are overwhelmingly SPVs with no other assets — LC preferred over parent guarantee during development. Some buyers use Surety Bonds — confirm with CFO/treasury.`,
        ...
        rec: key === 'red' ? 'Add Seller Development Security of $100K/MWac via LC from Qualified Institution (A-/A3, $2.5B+ net worth). Seller is always an SPV — no LC means no protection. Most critical credit term for buyer.' : key === 'seller' ? 'Increase to $75K/MWac minimum. Require replenishment obligation. If seller offers parent guarantee, confirm parent meets BBB-/Baa3 Required Credit Rating.' : 'Seller performance assurance acceptable. Confirm Qualified Institution definition matches market standard (A-/A3, $2.5B+).',
    ...
    getContent(p) {
      ...
      return {
        term: key === 'buyer' ? `<strong>Parent Guarantee from investment-grade rated parent entity</strong> (BBB-/Baa3 or better). No LC required. Guarantee cap is most-negotiated item — specify in term sheet. Performance assurance posted only if buyer drops below investment grade.` :
              key === 'market' ? `<strong>Parent Guarantee or LC at buyer's election</strong> — IG-rated buyers may provide parent guarantee; non-IG buyers post LC. Credit support amount, form, and draw conditions are intensely negotiated. Cap on guarantee typically most contentious item — lock it in at term sheet stage.` :
        ...
        bench: `Market standard: Buyer PA debate focuses on (1) amount, (2) form (LC vs guarantee), (3) draw conditions. IG buyers at parent level may avoid posting security. Cap on parent guarantee is typically the most-negotiated item — Best practice: specify it in the term sheet. Smaller buyers may find credit support a difficult financial burden — address early.`,
    ...
    getContent(p) {
      ...
      return {
        term: key === 'buyer' ? `<strong>Buyer consent required for all unaffiliated third-party assignments.</strong> Affiliate assignments permitted without consent. Collateral assignment to lender permitted (standard for project financing). Tax equity partner investment is not deemed an assignment. Change of control = assignment.` :
              key === 'market' ? `<strong>Standard market assignment provision.</strong> Neither party assigns without other's consent, except: (1) collateral assignment/pledge for project financing, (2) financial rights to qualified hedge counterparty. Change of control = assignment. Lender step-in rights and lender's right to assign post-step-in included.` :
              key === 'seller' ? `<strong>Seller may assign to any creditworthy counterparty without buyer consent.</strong> "Creditworthy" defined solely by seller. Financing terms can be difficult — lenders have varying requirements, and buyers may not realize they become exposed to developer's equity investors.` :
        ...
        bench: `Market standard: Mutual consent for unaffiliated assignments. Carve-outs: collateral assignment for financing (standard), financial rights to hedge counterparty, tax equity investment. Step-in rights for lender + lender's assignment right post-step-in. Note: different lenders have varying contract requirements — buyer should be aware of financing-related assignment complexity.`,
  ...
  fm: {
    getContent(p) {
      ...
      return {
        ...
        bench: `Market standard: 12-month FM cap with day-for-day term extension (not lost time). Excludes: supply chain, interconnection delays, permitting, economic reasons, inability to sell RECs. COVID-19 existing orders: NOT FM. Some sellers attempt to include supply chain and development delays — buyers should resist. Market experience: "FM does not mean there will be a project delay — COVID-19 has not caused significant impacts so far."`,
  ...
  eod: {
    getContent(p) {
      ...
      return {
        ...
        bench: `Market standard: Mutual symmetric EODs. Seller-specific EOD: Mechanical Availability < 70–75% in any 2 consecutive years. Payment: 3-day cure. Material breach: 30-day + 60-day extension. Bankruptcy: immediate. Dodd-Frank reporting: seller responsibility at seller's cost. Equal treatment of both parties required.`,
        ...
        rec: key === 'red' ? 'Add symmetric EODs including Mechanical Availability < 70–75% in 2 consecutive years as seller EOD. Without this, buyer cannot terminate even for persistent severe underperformance.' : key === 'seller' ? 'Equalize cure periods. Add 70–75% availability EOD trigger for seller. This is a market standard protection.' : 'Events of Default acceptable. Confirm Mechanical Availability trigger (70–75% / 2 consecutive years) is explicitly defined as seller EOD.',
  ...
  eterm: {
    getContent(p) {
      ...
      return {
        ...
        bench: `Market standard: Two-way MTM closeout — non-defaulting party receives MTM value. market also notes upside share mechanism: seller pays buyer only a % of monthly settlement in exchange for reduced strike price. This reduces buyer's downside in out-of-the-money periods. Useful risk mitigation tool — consider negotiating at term sheet stage.`,
  ...
  changeinlaw: {
    getContent(p) {
      const key = p <= 25 ? 'buyer' : p <= 50 ? 'market' : p <= 75 ? 'seller' : 'red';
      return {
        term: key === 'buyer' ? `<strong>Change in law has no effect on Fixed Price or Settlement Amount for either party.</strong> Market standard: neither the Fixed Price nor Settlement Amount calculation is affected by any Change in Law that alters either party's costs or the value of energy/environmental attributes. Both parties need fixed-price certainty.` :
              key === 'market' ? `<strong>Change in law does not allow either party to renegotiate Fixed Price or Settlement Amount.</strong> Buyers enter deals for fixed-price certainty; sellers need revenue certainty for project financing. Changes in law affect neither party's contractual obligations. Mutual certainty is the standard.` :
              key === 'seller' ? `<strong>Change in law triggers good-faith renegotiation</strong> at seller's request. If unresolved in 60 days, either party may terminate. Departs from market standard — any renegotiation right undermines fixed-price certainty that is core to VPPA value.` :
              `<strong>Change in law allows seller cost pass-through to buyer or unilateral seller exit.</strong> Directly contradicts market standard. Sellers need revenue certainty for financing; buyers need fixed-price certainty. Either pass-through or exit right makes the contract contingent — not fixed.`,
        bench: `Market standard (inflexible term): Neither Fixed Price nor Settlement Amount affected by any Change in Law altering either party's costs, operation, or environmental attribute values. Both parties need certainty. Market standard: "Changes in law should not allow either party to renegotiate the contract." Any renegotiation right undermines the fundamental fixed-price structure.`,
        impact: key === 'buyer' ? 'NO EFFECT EITHER PARTY' : key === 'market' ? 'MARKET STANDARD — NO EFFECT' : key === 'seller' ? 'RENEGOTIATION TRIGGER' : 'COST PASS-THROUGH / EXIT',
        impactsub: key === 'buyer' ? 'Fixed price locked<br>no change-in-law carve-out' : key === 'market' ? 'market standard<br>fixed price protected' : key === 'seller' ? 'Renegotiation trigger<br>undermines fixed price' : 'Pass-through or exit<br>market hard reject',
        rec: key === 'red' ? 'REJECT. market standard is explicit: change in law affects neither party\'s price or settlement obligation. Cost pass-through or seller exit right contradicts the fundamental fixed-price certainty that makes VPPAs financeable.' : key === 'seller' ? 'Remove renegotiation right. Market standard: changes in law should not allow either party to renegotiate. Fixed price certainty is essential for both buyer planning and seller project financing.' : 'Change in law provision acceptable and market-aligned.',
        reclabel: key === 'red' ? 'Priority 1 — Hard Reject' : key === 'seller' ? 'Priority — Remove Renegotiation' : 'Outcome — Market Aligned'
      };
    }
  },
  ...
  reputation: {
    getContent(p) {
      const key = p <= 25 ? 'buyer' : p <= 50 ? 'market' : p <= 75 ? 'seller' : 'red';
      return {
        term: key === 'buyer' ? `<strong>Full reputational disclosure suite.</strong> Seller representations and warranties covering: (1) public opposition to project, (2) environmental baseline and stewardship, (3) mitigation measures for wildlife and culturally sensitive properties. Ongoing disclosure obligation for changes during term. Cooperation provision on reputational concerns.` :
              key === 'market' ? `<strong>market standard reputational disclosures.</strong> VPPA contains seller reps/warranties on: public opposition, environmental baseline, environmental stewardship, wildlife and historically/culturally sensitive property mitigation. The VPPA is a long-term relationship with lasting reputational impacts on buyer — these provisions are increasingly expected.` :
        ...
        bench: `Market standard: Seller reps/warranties on public opposition, environmental baseline, stewardship, wildlife, and cultural property mitigation. Ongoing disclosure if material changes occur during term. Cooperation provision for reputational response. Market reality: "The VPPA is a long-term relationship with lasting reputational impacts." ESG compliance and non-energy reporting increasingly expected by Fortune 500 buyers.`,
        impact: key === 'buyer' ? 'FULL market SUITE' : key === 'market' ? 'MARKET STANDARD REPS' : key === 'seller' ? 'SIGNING DATE ONLY' : 'NO PROTECTION',
        ...
        rec: key === 'red' ? 'Add full reputational disclosure suite — public opposition, environmental baseline, stewardship, wildlife/cultural mitigation. VPPA is 12–20 year relationship. ESG issues that emerge mid-term create significant buyer reputational risk.' : key === 'seller' ? 'Add ongoing disclosure obligation for material changes to ESG factors during term. Signing-date-only reps leave buyer exposed for 12–20 years.' : 'Reputational provisions acceptable.',
        reclabel: key === 'red' ? 'Priority — Add Full Suite' : key === 'seller' ? 'Priority — Add Ongoing Disclosure' : 'Outcome — Acceptable'
      };
    }
  },
    ...
    getContent(p) {
      ...
      return {
        term: key === 'buyer' ? `<strong>Buyer receives all Environmental Attributes associated with Buyer's Share of project output,</strong> transferred monthly as Green-e certifiable RECs. Future Environmental Attributes (new REC standards, carbon credits) included subject to cost cap. Buyer has discretion to sell, hold, or retire — bears risk and benefit of future attribute price changes.` :
              key === 'market' ? `<strong>Environmental Attributes (project-specific RECs) delivered monthly</strong> as Green-e certifiable RECs for Buyer's Share of output. Future attribute rights subject to good-faith negotiation. Buyer has full discretion on sell/hold/retire — bears future price risk and benefit. Industry standard product.` :
              ...
              `<strong>Product definition minimal or RECs not Green-e certifiable.</strong> Seller retains all future attributes. Market standard: "In no event should buyers allow a seller to require a cost cap on RECs." Any cost cap applies only to future Environmental Attributes, not core RECs.`,
        bench: `Market standard: Monthly delivery of Green-e certifiable RECs for Buyer's Share. Future Environmental Attributes: seller certifies and obtains subject to cost cap ($/MW of Final Nameplate Capacity per year + aggregate cap). Environmental Attributes: ~100% to buyer. Incentives (ITC, capacity): ~100% to seller. Capacity/ancillaries: negotiated. Buyer bears risk and benefit of future attribute price changes.`,
  ...
  recs: {
    getContent(p) {
      ...
      return {
        ...
        bench: `Market standard: RECs registered in applicable tracking system (WREGIS, ERCOT, PJM-GATS). REC Arbitrage = supplemental/replacement RECs when seller cannot deliver. Specify technology type (wind/solar) and region at term sheet stage. Some buyers request 100–120% replacement to compensate for lost marketing claims from project-specific RECs. Buyers may choose cash LDs in lieu of replacement RECs.`,
    ...
    getContent(p) {
      ...
      return {
        term: key === 'buyer' ? `<strong>Buyer-favorable: At buyer's option, seller monetizes buyer's share of project capacity and ancillary services</strong> and pays buyer a percentage of net revenues. Environmental Attributes: 100% to buyer. Incentives (ITC/PTC): 100% to seller (standard). Capacity/ancillaries: buyer receives % of net revenues.` :
              key === 'market' ? `<strong>market standard allocation.</strong> Environmental Attributes: ~100% buyer. Tax incentives (ITC/PTC), local credits, rebates, grants: ~100% seller. Facility Attributes (capacity, ancillary services): negotiated — sellers increasingly push for revenue sharing rather than full buyer allocation.` :
        ...
        bench: `Market standard: Environmental Attributes = buyer. Tax incentives = seller. Capacity/ancillaries = negotiated. Best practice: work with cross-functional teams (utility, facilities) — demonstrating new capacity via PPA may provide leverage on utility rates. Buyers increasingly want to share capacity market value as non-environmental attribute markets evolve. Seller notification obligation for ITC/incentive changes is a baseline protection.`,
        impact: key === 'buyer' ? 'CAPACITY REVENUE SHARE' : key === 'market' ? 'MARKET STANDARD SPLIT' : key === 'seller' ? 'ALL TO SELLER — NO NOTICE' : 'NO PROVISIONS',
        ...
        rec: key === 'red' ? 'Add notification obligation for material incentive changes (ITC transfers, state incentive changes). ITC transfer without notice can affect construction timeline and quality. Add first right of refusal on capacity and ancillary revenue sharing.' : key === 'seller' ? 'Add seller notification obligation for ITC/incentive changes. Consider negotiating % of capacity and ancillary service revenues — These can provide meaningful utility rate leverage.' : 'Incentive allocation acceptable. Confirm seller notification obligation for material incentive changes is included.',
  ...
  govlaw: {
    getContent(p) {
      const key = p <= 25 ? 'buyer' : p <= 50 ? 'market' : p <= 75 ? 'seller' : 'red';
      return {
        term: key === 'buyer' ? `<strong>New York governing law.</strong> market standard for financial energy contracts. NY courts have deep ISDA/financial contract jurisprudence. Dispute resolution via AAA arbitration, expedited track. Most buyers and lenders prefer NY law.` :
              key === 'market' ? `<strong>New York governing law.</strong> Market standard: New York law. Most parties agree on NY law for financial instruments. The alternative — project state jurisdiction — is not preferred by most buyers or lenders.` :
              key === 'seller' ? `<strong>Governing law is seller's home state</strong> or project state jurisdiction. Less developed financial contract case law, potential home-court advantage for seller. market and most lenders prefer NY law.` :
        ...
        bench: `Market standard: New York law. Market standard: "Since these are financial instruments, most parties agree on the use of New York law. The alternative is to use the law of the jurisdiction where the project is located, but most buyers and lenders do not prefer that." Texas law may be acceptable for ERCOT-specific deals; Delaware is also common.`,
        impact: key === 'buyer' ? 'NEW YORK — STANDARD' : key === 'market' ? 'NEW YORK — MARKET ALIGNED' : key === 'seller' ? "SELLER'S STATE" : 'NOT SPECIFIED',
        impactsub: key === 'buyer' ? 'NY law, ISDA-aligned<br>lender preferred' : key === 'market' ? 'NY law market standard<br>deep financial contract law' : key === 'seller' ? "Seller's home state<br>lenders may resist" : 'No jurisdiction defined<br>enforcement risk',
        rec: key === 'red' ? 'Specify New York governing law. market standard — most lenders will require NY law for project financing. Undefined jurisdiction creates enforcement risk.' : key === 'seller' ? 'Push for New York law. Seller home state jurisdiction is non-standard — lenders and financiers typically require NY law.' : 'Governing law acceptable.',
        reclabel: key === 'red' ? 'Priority — Specify NY Law' : key === 'seller' ? 'Priority — Push for NY' : 'Outcome — Market Aligned'
      };
    }
  },
    ...
    getContent(p) {
      ...
      return {
        term: key === 'buyer' ? `<strong>Confidentiality coterminous with agreement + 3-year post-termination tail.</strong> Contents of term sheet and VPPA may not be shared beyond buyer, seller, and their advisors. Strong protection for pricing, volumes, and party identities. Financing party disclosure requires recipient NDA.` :
              key === 'market' ? `<strong>Market standard: Term sheet subject to NDA entered into between parties.</strong> VPPA will have its own confidentiality provisions. Contents not shared beyond buyer, seller, and advisors. Best practice: VPPA itself should contain separate, standalone confidentiality provisions beyond term sheet NDA.` :
        ...
        bench: `Market standard: Term sheet subject to NDA; VPPA contains standalone confidentiality provisions. Standard carve-outs: legal/regulatory disclosure, financing disclosures (under NDA), board/advisor sharing. Pricing and buyer identity must be protected throughout term + 2-year minimum tail.`,
  ...
  excl: {
    getContent(p) {
      ...
      return {
        ...
        bench: `Market standard: 60 days from term sheet signing. Scope: prevents advertising the project (seller) or searching for a new project (buyer) for the same capacity. Mutual exclusivity applies to both parties. 60 days is market norm; longer periods require compelling justification.`,
        impact: key === 'buyer' ? 'NO EXCLUSIVITY' : key === 'market' ? '60-DAY MUTUAL' : key === 'seller' ? '90+ DAYS — BROAD' : 'OPEN-ENDED',
        impactsub: key === 'buyer' ? 'Buyer free to negotiate<br>competing projects' : key === 'market' ? '60-day mutual, market norm<br>both parties restricted' : key === 'seller' ? '90+ days too long<br>overly broad scope' : 'No end date defined<br>buyer locked indefinitely',
    ...
    getContent(p) {
      ...
      return {
        term: key === 'buyer' ? `<strong>Each party bears its own legal, diligence, and advisory costs</strong> — no cost shifting in any scenario. Market standard. Legal fees typically $50–150K+ per side on VPPA negotiations.` :
              ...
              key === 'seller' ? `<strong>Buyer responsible for certain seller costs</strong> if buyer terminates during exclusivity period — creates de facto break fee. This is non-standard and should be rejected.` :
        ...
        bench: `Market standard: Each party bears its own expenses. Any cost-shifting triggered by buyer termination is non-standard and should be rejected. Break fee provisions are not standard. Legal fees: $50–150K+ per side is typical.`,
        impact: key === 'buyer' ? 'EACH BEARS OWN' : key === 'market' ? 'EACH BEARS OWN' : key === 'seller' ? 'BUYER BREAK FEE' : 'BUYER PAYS ALL',
        impactsub: key === 'buyer' ? 'No cost recovery risk<br>market standard' : key === 'market' ? 'market standard<br>no cost shifting' : key === 'seller' ? 'Buyer owes seller costs<br>if terminates — break fee' : 'Buyer pays seller costs<br>regardless of fault',
        rec: key === 'red' ? 'Reject cost-shifting provisions. Buyer liability for seller costs is non-standard and creates unjust deterrent to exercising legitimate termination rights (OCOD, CP failure). Market standard: each party bears own expenses.' : key === 'seller' ? 'Remove break fee. Cost-shifting during exclusivity is non-standard.' : 'Expense allocation acceptable.',
  ...
  acct: {
    getContent(p) {
      ...
      return {
        ...
        bench: `Engage accounting team early. Key accounting decisions: (1) Buyer's Share as % of capacity (not fixed MWh) — avoids Guaranteed Annual Production risk; (2) Cap at 90% — avoids lease treatment; (3) Fixed-for-floating structure not physical delivery — preserves ASC 815 derivative treatment. Review accounting treatment with your Big 4 advisor before finalizing term sheet structure.`,
  ...
  publicity: {
    getContent(p) {
      const key = p <= 25 ? 'buyer' : p <= 50 ? 'market' : p <= 75 ? 'seller' : 'red';
      return {
        term: key === 'buyer' ? `<strong>Mutual prior written consent required for all press releases and public statements.</strong> Buyer name and contract relationship: not publicized without buyer consent. Financing party name: not publicized without financing party consent. Buyers may wish to have exclusive naming or other publicity rights.` :
              key === 'market' ? `<strong>Market standard: Buyer's name and contract relationship not publicized other than by or with consent of Buyer.</strong> No financing party's name publicized without consent. Market standard: "Developers should always consult with the buyer before any public statements are made regarding the project." Often includes mutual consent prior to any press release identifying the other party.` :
              key === 'seller' ? `<strong>Seller may issue press release or public statement announcing buyer and project</strong> without buyer pre-approval. Seller financing roadshows are common — but must still require buyer pre-approval under market standard.` :
        ...
        bench: `Market standard: Buyer name and contract relationship not publicized without buyer consent. Financing party name protected similarly. Market practice: developers should always consult with the buyer before any public statements are made. Mutual consent before press releases is increasingly standard. Buyers may seek exclusive naming rights.`,
        impact: key === 'buyer' ? 'MUTUAL CONSENT' : key === 'market' ? 'MARKET STANDARD' : key === 'seller' ? 'SELLER CAN ANNOUNCE' : 'NO RESTRICTIONS',
        impactsub: key === 'buyer' ? 'Mutual prior written consent<br>exclusive naming option' : key === 'market' ? 'Buyer consent required<br>market standard' : key === 'seller' ? 'Seller may announce buyer<br>without pre-approval' : 'Fully exposed<br>competitive + ESG risk',
        rec: key === 'red' ? 'Add mutual consent requirement. Market standard: developer should always consult buyer before public statements. Unrestricted announcement exposes pricing, strategy, and buyer identity to competitors and potential activists.' : key === 'seller' ? 'Require buyer pre-approval for all press releases. Seller financing roadshows are common — buyer should control its own announcement timing and messaging.' : 'Publicity rights acceptable.',
        reclabel: key === 'red' ? 'Priority — Add Consent Req.' : key === 'seller' ? 'Priority — Require Pre-Approval' : 'Outcome — Market Aligned'
      };
    }
  }
